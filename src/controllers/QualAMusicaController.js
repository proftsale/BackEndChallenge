...
module.exports = {
  async index(request, response) {
    ...
    temperature = await axios.get(weatherApiURL)
    ...
    .get(spotifyBaseApiURL + `playlists/${playlistHypada}/tracks?fields=items(track(name,popularity,external_urls))`, {
      headers: {
        Authorization: `Bearer ${spotifyApiKey}`,
      },
    })
    ...
    musicasPlaylist.map(({track}, index) => {
        if(track.popularity > melhorMusica.popularity)
            melhorMusica = track
    }) 
